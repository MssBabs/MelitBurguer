...
@Component({
  selector: 'jhi-productos-pedido',
  templateUrl: './productos-pedido.component.html'
})
export class ProductosPedidoComponent implements OnInit, OnDestroy {
  currentAccount: any;
  ...
  transition() {
    this.router.navigate(['/productos-pedido'], {
      queryParams: {
        page: this.page,
        size: this.itemsPerPage,
  ...
  ngOnInit() {
    ...
    this.registerChangeInProductosPedidos();
  }
  ...
  trackId(index: number, item: IProductosPedido) {
    return item.id;
  }

  registerChangeInProductosPedidos() {
    this.eventSubscriber = this.eventManager.subscribe('productosPedidoListModification', response => this.loadAll());
  }
