...
export class RecompensaComponent implements OnInit, OnDestroy {
  currentAccount: any;
  ...
  loadAll() {
    this.recompensaService
      ...
      .subscribe(
        (res: HttpResponse<IRecompensa[]>) => this.paginateRecompensas(res.body, res.headers),
  ...
  ngOnInit() {
    ...
    this.registerChangeInRecompensas();
  }
  ...
  trackId(index: number, item: IRecompensa) {
    return item.id;
  }

  registerChangeInRecompensas() {
    this.eventSubscriber = this.eventManager.subscribe('recompensaListModification', response => this.loadAll());
  }
  ...
  protected paginateRecompensas(data: IRecompensa[], headers: HttpHeaders) {
